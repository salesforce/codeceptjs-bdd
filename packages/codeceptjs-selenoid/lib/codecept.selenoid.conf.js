...
function config() {
    function isSelenoidRequested() {
        return gProfile && gProfile.match('selenoid:[a-zA-Z]');
    }
    ...
    function getBrowserInfo() {
        ...
        if(info.length < 3) {
            if(info[1] === 'chrome' || info[1] === 'firefox') {
                throw new Error(`${emoji.get('warning')}  ` + 
                chalk.yellow.bold('Selenoid Version must be required. ' + 
                'The "profile" must be in the format of "selenoid:<chrome|firefox>:<version>' + 
                ' but found "' + gProfile + '"'));
            } else {
                 throw new Error(`${emoji.get('warning')}  ` + 
                 chalk.yellow.bold('Selenoid Browser is either missing or incorrect. ' +
                 'The "profile" must be in the format of "selenoid:<chrome|firefox>:<version>'+ 
                 ' but found "' + gProfile + '"')); 
            }
    ...
    if (isSelenoidRequested()) {
        ...
        console.info(`${emoji.get('sun_behind_cloud')}  ` + 
        chalk.yellow.bold('Tests are running on Selenoid: ') + chalk.blue.bold(JSON.stringify(browserInfo) + '\n'));
